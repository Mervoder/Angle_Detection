...
void LSM303_Read_Accel()
{
	...
	//{
		...
		AccelZ = ((int16_t)((acHData[0] << 8) | acLData[0]) >> Shift );
		AccelZ= (float)AccelZ*multipler;
		...
		AccelX =  ((int16_t)((acHData[0] << 8) | acLData[0]) ) >> Shift;

		AccelX= (float)AccelX*multipler;
		...
		AccelY =  ((int16_t)((acHData[0] << 8) | acLData[0]) ) >> Shift;
		AccelY= (float)AccelY*multipler;
